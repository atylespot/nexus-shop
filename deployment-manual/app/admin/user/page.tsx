...
export default function UserPage() {
  const [activeTab, setActiveTab] = useState<"create" | "permission" | "performance">("permission");
  ...
  const [perfUserId, setPerfUserId] = useState<number | null>(null);
  ...
  useEffect(() => {
    try {
      ...
      if (t === 'create' || t === 'permission' || t === 'performance') setActiveTab(t as any);
    } catch {}
  }, []);
  useEffect(() => {
  ...
  return (
    <div className="bg-white rounded-lg border shadow-sm">
      <div className="flex gap-2 p-3 border-b">
        ...
        <button
          className={`px-4 py-2 text-sm rounded-md ${activeTab === 'performance' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'text-gray-600 hover:bg-gray-50'}`}
        ...
        <div className="p-4">
          <div className="mb-3 flex gap-2 items-center">
              ...
              onChange={async (e) => {
                ...
                if (id) {
                  try {
                    ...
                    if (perfFrom && perfTo) { qs.set('from', perfFrom); qs.set('to', perfTo); }
                    const r = await fetch(`/api/users/${id}/performance?${qs.toString()}`, { cache: 'no-store' });
            ...
            >
              ...
              {users.map(u => (
                <option key={u.id} value={u.id}>{u.name} ({u.userId || u.email})</option>
              ))}
            </select>
            <input type="date" value={perfFrom} onChange={(e)=>setPerfFrom(e.target.value)} className="border rounded px-3 py-2" />
            <input type="date" value={perfTo} onChange={(e)=>setPerfTo(e.target.value)} className="border rounded px-3 py-2" />
            <button
              onClick={async ()=>{
                ...
                const qs = new URLSearchParams();
                if (perfFrom && perfTo) { qs.set('from', perfFrom); qs.set('to', perfTo); }
                const r = await fetch(`/api/users/${perfUserId}/performance?${qs.toString()}`, { cache: 'no-store' });
                if (r.ok) setPerfData(await r.json());
              }}
            ...
            <button
              onClick={()=>{
                if (!perfData) return;
                const rows = [
                  ['Time','Action','Target Type','Target ID'] as string[],
                  ...((perfData.recent||[]).map((a:any)=>[
                ...
                link.href = url;
                link.download = `user_${perfUserId}_activities.csv`;
          ...
          {!perfUserId && <p className="text-gray-500 text-sm">Select a user to view performance.</p>}
          {perfUserId && !perfData && <p className="text-gray-500 text-sm">Loading...</p>}
          {perfData && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="border rounded p-3">
                <h4 className="font-semibold mb-2">Summary</h4>
                <div className="text-sm space-y-1">
                  <div>Today: <span className="font-medium">{perfData.counts?.today ?? 0}</span></div>
                  <div>This Week: <span className="font-medium">{perfData.counts?.thisWeek ?? 0}</span></div>
                  <div>This Month: <span className="font-medium">{perfData.counts?.thisMonth ?? 0}</span></div>
                  <div>Total: <span className="font-medium">{perfData.counts?.total ?? 0}</span></div>
                </div>
              </div>
              <div className="border rounded p-3">
                <h4 className="font-semibold mb-2">By Action</h4>
                <ul className="text-sm list-disc ml-5 space-y-1">
                  {(perfData.byAction || []).map((row: any) => (
                    <li key={row.action} className="flex justify-between"><span>{row.action}</span><span className="font-medium">{row._count?.action || 0}</span></li>
                  ))}
                </ul>
              </div>
                ...
                <ul className="text-sm space-y-1 max-h-64 overflow-auto">
                  {(perfData.recent || []).map((a: any) => (
                    <li key={a.id} className="flex items-center gap-2">
                      <span className="inline-block w-24 text-xs text-gray-500">{new Date(a.createdAt).toLocaleString()}</span>
                      <span className="px-2 py-0.5 text-xs bg-emerald-50 text-emerald-700 rounded border border-emerald-200">{a.action}</span>
                      <span className="text-gray-700">{a.targetType || '-'} {a.targetId ? `#${a.targetId}` : ''}</span>
                    </li>
                  ))}
                </ul>
              </div>
...
function SectionPermissionEditor({ allPermissions, allSectionsLabel, roleMap, setRoleMap }: { allPermissions: {id:number; resource:string; action:string}[]; allSectionsLabel: {key:string; label:string}[]; roleMap: Record<string, boolean>; setRoleMap: (f: any)=>void }) {
  ...
  // Helper: is All selected for current section
  const isAllSelected = actions.length > 0 && actions.every(p => !!roleMap[`${p.id}`] || !!roleMap[`${p.resource}:${p.action}`]);

  // Toggle all actions for the current section
  const toggleAllForSection = (checked: boolean) => {
    setRoleMap((prev: any) => {
      const next: Record<string, boolean> = { ...prev };
      actions.forEach(p => {
        // Prefer string key to be consistent when permissions may not exist yet
        next[`${p.resource}:${p.action}`] = checked;
        // Also mirror numeric id key if present in case UI is reading it
        if (p.id && p.id !== -1) next[`${p.id}`] = checked;
      });
      return next;
    });
  };
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div className="md:col-span-3 border rounded-md p-3">
        <div className="flex items-center justify-between mb-2">
          <h4 className="font-medium">{availableSections.find(s=>s.key===sectionKey)?.label || 'Permissions'}</h4>
          {/* All toggle for this section */}
          <label className="flex items-center gap-2 text-sm">
            <span className="text-gray-700">All</span>
            <input type="checkbox" checked={isAllSelected} onChange={(e)=>toggleAllForSection(e.target.checked)} />
          </label>
        </div>
