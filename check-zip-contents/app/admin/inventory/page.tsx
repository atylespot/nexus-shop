...
export default function InventoryPage() {
  ...
  // Load categories from products (fallback)
    ...
    try {
      ...
      if (savedProducts) {
        ...
        const uniqueCategories = Array.from(
          new Set(
            products
              .filter((product: any) => product.categoryName && typeof product.categoryName === 'string')
              .map((product: any) => product.categoryName as string)
          )
        ...
        console.log('Categories loaded from products:', uniqueCategories.length);
      } else {
        setCategories([]);
      }
    } catch (error) {
  ...
  const loadInventoryFromAPI = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const inventoryData: InventoryItem[] = data.map((item: any, index: number) => ({
          id: `${item.id?.toString() || item.productId?.toString() || 'unknown'}-${index}`,
  ...
  const loadInventoryFromProducts = () => {
    ...
    if (savedProducts) {
      try {
        ...
        const inventoryData: InventoryItem[] = products.map((product: any, index: number) => ({
          id: `${product.id?.toString() || 'unknown'}-${index}`,
        ...
        const lightweightData = inventoryData.map((item, index) => ({
          id: item.id,
          productName: item.productName,
